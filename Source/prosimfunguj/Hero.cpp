...
void AHero::Tick(float DeltaTime)
{
	...
	GEngine->AddOnScreenDebugMessage(-1,0.f,FColor::Emerald,FString::Printf(TEXT("Pocet Kyticek: %i"),Kyticky),true);
	...
	int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	FString Pavel = OtherActor->GetName().ToLower();
	if (Pavel.Contains("killbox"))
	{
		FVector Location = GetActorLocation();
		Location.Set(0,0,0);
		SetActorLocation(Location);
	}
	if (Pavel.Contains("kyticka"))
	{
		Kyticky += 1;
		OtherActor->Destroy();
	}
